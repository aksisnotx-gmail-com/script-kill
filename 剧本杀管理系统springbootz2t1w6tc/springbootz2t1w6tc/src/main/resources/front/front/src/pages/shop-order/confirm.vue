  ...
  <div :style='{"width":"100%","padding":"80px 10%","margin":"10px auto","position":"relative","background":"#F6F6F6"}'>
    ...
    <div class="buy" v-if="seat!=1 && tableData.length > 0">
      ...
      <template v-else>
        ...
        <el-button type="success" @click="payClick">余额支付</el-button>
        <el-button type="primary" @click="alipayClick">沙箱支付</el-button>
      </template>
    </div>
    <div class="buy" v-if="seat==1 && tableData.length > 0">
      <template v-if="type == 2">
      ...
      <template v-else>
        <span style="color: red;margin-right: 10px;font-weight: 700;">总价：<span :style='{"fontSize":"12px"}'>￥</span>{{totalPrice}}</span>
        <el-button type="success" @click="xzPayClick">余额支付</el-button>
        <el-button type="primary" @click="xzAlipayClick">沙箱支付</el-button>
      </template>
    </div>
  </div>
</template>
                    ...
                    // 更新库存信息
                     this.$http.post(`${item.tablename}/update`, data).then( res => {
                        ...
                        let order = {
                         ...
                         this.$http.post('orders/add', order).then( res => {
                             this.$http.get('orders/alipay', {params: {tradeno: order.orderid, totalamount: order.total, subject: order.goodname}}).then( res => {
								if (res.data.code == 0) {
