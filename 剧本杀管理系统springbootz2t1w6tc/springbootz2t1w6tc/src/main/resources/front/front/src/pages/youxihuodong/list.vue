...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('youxihuodong','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"10px","background":"#FFDFA6","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/youxihuodongAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"padding":"20px 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"1px solid #5FE3EB","width":"200px","padding":"10px","overflow":"hidden","borderRadius":"10px","height":"250px"}' class="img-box">
				<img @click.stop="imgPreView(item.huodongfengmian)" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-if="item.huodongfengmian && item.huodongfengmian.substr(0,4)=='http'" :src="item.huodongfengmian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.huodongfengmian?item.huodongfengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-else :src="baseUrl + (item.huodongfengmian?item.huodongfengmian.split(',')[0]:'')" class="image" />
			</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`youxihuodong/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/youxihuodongDetail', query: params});
      },
