...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>剧本名称：</div>
        <el-input v-model="formSearch.jubenmingcheng" placeholder="剧本名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('jubenxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"10px","background":"#FFDFA6","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jubenxinxiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"padding":"20px 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"1px solid #5FE3EB","width":"200px","padding":"10px","overflow":"hidden","borderRadius":"10px","height":"250px"}' class="img-box">
				<img @click.stop="imgPreView(item.jubenfengmian)" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-if="item.jubenfengmian && item.jubenfengmian.substr(0,4)=='http'" :src="item.jubenfengmian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.jubenfengmian?item.jubenfengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-else :src="baseUrl + (item.jubenfengmian?item.jubenfengmian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"110px","padding":"0 10px 0 0","flex":"1"}'>
				<div class="title">{{item.jubenmingcheng}}</div>
				<div class="title">{{item.jubenfenlei}}</div>
		      ...
		      <div class="desc" v-html="item.jubenjianjie"></div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.jubenmingcheng != '') searchWhere.jubenmingcheng = '%' + this.formSearch.jubenmingcheng + '%';
        if (this.curFenlei != '全部') searchWhere.jubenfenlei = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`jubenxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jubenxinxiDetail', query: params});
      },
