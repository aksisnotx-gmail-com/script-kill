	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="名称" prop="mingcheng">
					<el-input v-model="ruleForm.mingcheng" placeholder="名称" clearable  :readonly="ro.mingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="名称" prop="mingcheng">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="道具分类" prop="daojufenlei">
					<el-select :disabled="ro.daojufenlei" v-model="ruleForm.daojufenlei" placeholder="请选择道具分类" >
						<el-option
							v-for="(item,index) in daojufenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="道具分类" prop="daojufenlei">
					<el-input v-model="ruleForm.daojufenlei"
						placeholder="道具分类" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="适玩人群" prop="shiwanrenqun">
					<el-input v-model="ruleForm.shiwanrenqun" placeholder="适玩人群" clearable  :readonly="ro.shiwanrenqun"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="适玩人群" prop="shiwanrenqun">
					<el-input v-model="ruleForm.shiwanrenqun" placeholder="适玩人群" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="游戏介绍" prop="youxijieshao">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="游戏介绍"
					  v-model="ruleForm.youxijieshao" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.youxijieshao" label="游戏介绍" prop="youxijieshao">
					<span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.youxijieshao}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="详情内容" prop="xiangqingneirong">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.xiangqingneirong" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.xiangqingneirong" label="详情内容" prop="xiangqingneirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.xiangqingneirong"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='daojufenlei'){
							this.ruleForm.daojufenlei = obj[o];
							this.ro.daojufenlei = true;
							continue;
						}
						if(o=='shiwanrenqun'){
							this.ruleForm.shiwanrenqun = obj[o];
							this.ro.shiwanrenqun = true;
							continue;
						}
						...
						if(o=='xiangqingneirong'){
							this.ruleForm.xiangqingneirong = obj[o];
							this.ro.xiangqingneirong = true;
							continue;
						}
						if(o=='youxijieshao'){
							this.ruleForm.youxijieshao = obj[o];
							this.ro.youxijieshao = true;
							continue;
						}
						if(o=='thumbsupnum'){
            ...
            this.$http({
				url: `option/daojufenlei/daojufenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.daojufenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.xiangqingneirong = this.ruleForm.xiangqingneirong.replace(reg,'../../../springbootz2t1w6tc/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `youxidaoju/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.youxidaojuCrossAddOrUpdateFlag = false;
						 this.parent.search();
