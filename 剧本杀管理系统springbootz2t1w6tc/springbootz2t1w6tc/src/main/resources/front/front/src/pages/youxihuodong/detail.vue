...
<div>
	<div :style='{"padding":"20px","margin":"64px auto 10px","borderRadius":"0 0 30px 30px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"15px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/youxihuodong"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"minHeight":"550px","width":"45%","padding":"0 20px","position":"relative","background":"#fff","display":"flex"}'>
			...
			<div class="info" :style='{"padding":"0","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>倒计时</div>
					<count-down v-if="endTime&&startTime"
						:style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'
						:startTime="startTime"
						:endTime="endTime"
						:tipText="'距离开始 '"
						tipTextEnd="距离结束 "
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>游戏角色</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'>{{detail.youxijiaose}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>活动信息</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'>{{detail.huodongxinxi}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>简介</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'>{{detail.jianjie}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>活动费用</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'>{{detail.huodongfeiyong}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('youxihuodong','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFDFA680","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('youxihuodong','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
					<!-- crossOptButtonStatusColumns  -->
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('youxihuodong','活动报名')" @click="onAcross('huodongbaoming','','','','')" type="warning">活动报名</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.jubenmingcheng;
              this.detailBanner = this.detail.huodongfengmian ? this.detail.huodongfengmian.split(",") : [];
      ...
      async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
        ...
        localStorage.setItem('crossTable',`youxihuodong`);
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('youxihuodong/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'youxihuodong', userid: localStorage.getItem('frontUserid')}}).then(res => {
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("frontToken")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'youxihuodong', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'youxihuodong', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      getDiscussList(page) {
        this.$http.get('discussyouxihuodong/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussyouxihuodong/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussyouxihuodong/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/youxihuodongAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此游戏活动？')
			  .then(_ => {
			    this.$http.post('youxihuodong/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
